...
class JsonEligibilityProvider(EligibilityProvider):
    ...
    def check(self, request: EligibilityRequest) -> EligibilityResult:
        ...
        return EligibilityResult(
            ...
            copay=Decimal(record["copay"]) if record.get("copay") else None,
            deductible_remaining=(
                Decimal(record["deductible_remaining"])
                if record.get("deductible_remaining")
                else None
            ),
...
def generate_270(request: EligibilityRequest, control_number: int = 1) -> str:
    ...
    segments = [
        ...
        "HL*3*2*22*0",
        f"NM1*IL*1*SUBSCRIBER*****MI*{request.member_id}",
        f"DTP*291*D8*{dos}",
        "EQ*30",
        f"SE*11*{control_number:04d}",
    ]
    return "~\n".join(segments) + "~\n"
