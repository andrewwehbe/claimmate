/**
 * Local CARC/RARC reference, mirrored from data/rules/carc_rarc.json.
 * Keep in sync with that file — it is the single source of truth for the
 * backend; this module exists so tooltips work with zero network calls.
 */

import type { DenialCategory } from "../types";
...
export const CARC: Record<string, CarcEntry> = {
  "1": {
...
export function carcDescription(code: string): string {
  return CARC[code]?.description ?? "Unknown adjustment reason code.";
}

export function rarcDescription(code: string): string {
  return RARC[code] ?? "Unknown remark code.";
}
